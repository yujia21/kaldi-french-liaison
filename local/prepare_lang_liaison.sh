#!/bin/bash
#Create an alternate L_liaison.fst
#in srcdir, put liaisons.txt
#in data/lang put phones.txt with liaisons in it
...
#Add liaison phones to files
utils/prepare_phones_add_liaisons.sh $tmpdir $dir
...
if $silprob; then
  ...
  utils/make_lexicon_fst_silprob_liaison.pl $tmpdir/lexiconp_silprob_disambig.txt $srcdir/silprob.txt $silphone '#'$ndisambig | \
     sed 's=\#[0-9][0-9]*=<eps>=g' | \
     fstcompile --isymbols=$dir/phones.txt --osymbols=$dir/words.txt \
     --keep_isymbols=false --keep_osymbols=false |   \
     fstarcsort --sort_type=olabel > $dir/L.fst || exit 1;
else
  utils/make_lexicon_fst_liaison.pl --pron-probs $tmpdir/lexiconp.txt $sil_prob $silphone | \
    fstcompile --isymbols=$dir/phones.txt --osymbols=$dir/words.txt \
    --keep_isymbols=false --keep_osymbols=false | \
     fstarcsort --sort_type=olabel > $dir/L.fst || exit 1;
fi
...
if $silprob; then
  ...
  utils/make_lexicon_fst_liaison.pl --pron-probs $tmpdir/lexiconp_disambig.txt $sil_prob $silphone '#'$ndisambig | \
     fstcompile --isymbols=$dir/phones.txt --osymbols=$dir/words.txt \
     --keep_isymbols=false --keep_osymbols=false |   \
     fstaddselfloops  $dir/phones/wdisambig_phones.int $dir/phones/wdisambig_words.int | \
     fstarcsort --sort_type=olabel > $dir/L_disambig.fst || exit 1;
...
! utils/validate_lang_liaison.pl $dir && echo "$(basename $0): error validating output" &&  exit 1;
