...
if [ $stage -le 0 ]; then
  if [ -f "$graphdir/num_pdfs" ]; then
    [ "`cat $graphdir/num_pdfs`" -eq `am-info --print-args=false $model | grep pdfs | awk '{print $NF}'` ] || \
      { echo "Missmatch in number of pdfs with $model"; exit 1; }
  fi
...
if [ $stage -le 2 ]; then
	rm -f $dir/decodefull.txt
	echo "Decoding full";
	for i in $(seq 1 $nj); do
	  local/clean_decode_log.sh $dir/log/decode.$i.log; # $data/split2/$i/text;
          cat $dir/log/decode.$i.log.clean.norep.txt >> $dir/decodefull.txt
	done
	compute-wer --text --mode=present ark:$data/text ark:$dir/decodefull.txt >> $dir/wer_liaison.txt
fi
